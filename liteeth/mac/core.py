...
class LiteEthMACCore(Module, AutoCSR):
    def __init__(self, phy, dw, endianness="big",
        ...
        cd_eth_tx = phy.cd_eth_tx
        cd_eth_rx = phy.cd_eth_rx
        ...
        self.submodules += ClockDomainsRenamer(cd_eth_tx)(tx_gap_inserter)
        ...
        if isinstance(phy, LiteEthPHYModel):
            ...
            self.submodules += ClockDomainsRenamer(cd_eth_tx)(preamble_inserter)
            self.submodules += ClockDomainsRenamer(cd_eth_rx)(preamble_checker)
            ...
            self.submodules += ClockDomainsRenamer(cd_eth_tx)(crc32_inserter)
            self.submodules += ClockDomainsRenamer(cd_eth_rx)(crc32_checker)

            # DW aligner [unused]
            dw_aligner = aligner.LiteEthMACAligner(phy.dw)
            self.submodules += ClockDomainsRenamer(cd_eth_tx)(dw_aligner)
            ...
            # Error counters
            self.submodules.ps_preamble_error = PulseSynchronizer(cd_eth_rx, "sys")
            self.submodules.ps_crc_error = PulseSynchronizer(cd_eth_rx, "sys")
        ...
        if with_padding:
            ...
            self.submodules += ClockDomainsRenamer(cd_eth_tx)(padding_inserter)
            self.submodules += ClockDomainsRenamer(cd_eth_rx)(padding_checker)
        ...
        if dw != 8:
            ...
            self.submodules += ClockDomainsRenamer(cd_eth_tx)(tx_last_be)
            self.submodules += ClockDomainsRenamer(cd_eth_rx)(rx_last_be)
        ...
        if dw != phy.dw:
            ...
            rx_converter = stream.StrideConverter(eth_phy_description(phy.dw),
                                                  ...
                                                  reverse=reverse)
            self.submodules += ClockDomainsRenamer(cd_eth_tx)(tx_converter)
            self.submodules += ClockDomainsRenamer(cd_eth_rx)(rx_converter)
        ...
        self.submodules += ClockDomainsRenamer({"write": "sys", "read": cd_eth_tx})(tx_cdc)
        self.submodules += ClockDomainsRenamer({"write": cd_eth_rx, "read": "sys"})(rx_cdc)
